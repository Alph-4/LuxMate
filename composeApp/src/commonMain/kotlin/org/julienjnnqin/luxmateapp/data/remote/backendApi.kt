...
class KtorbackendApi(private val client: HttpClient, private val tokenStore: TokenStore) : backendApi {
    companion object {
    ...
    private suspend fun tryRefresh(): TokenResponse? {
        ...
        return try {
            client.post("$API_URL/auth/refresh") {
    ...
    override suspend fun refreshToken(refreshToken: String): TokenResponse {
        return client.post("$API_URL/auth/refresh") {
            header("Content-Type", ContentType.Application.Json.contentType)
    ...
    override suspend fun getPersonas(): List<Persona> {
        try {
            ...
            val resp: HttpResponse = client.get("$API_URL/personas") {
            ...
            if (resp.status == HttpStatusCode.Unauthorized) {
                ...
                return client.get("$API_URL/personas") {
    ...
    override suspend fun getPersonaById(personaId: String): Persona {
        try {
            val token = tokenStore.getAccessToken()
            val resp: HttpResponse = client.get("$API_URL/personas/$personaId") {
                if (!token.isNullOrEmpty()) header("Authorization", "Bearer $token")
            }
            if (resp.status == HttpStatusCode.Unauthorized) {
                val newTokens = tryRefresh() ?: throw Exception("Unauthorized")
                tokenStore.save(newTokens)
                return client.get("$API_URL/personas/$personaId") {
                    header("Authorization", "Bearer ${newTokens.accessToken}")
                }.body()
            }
            return resp.body()
    ...
    override suspend fun getSessions(): List<ChatSession> {
        try {
            val token = tokenStore.getAccessToken()
            val resp: HttpResponse = client.get("$API_URL/chat/sessions") {
                if (!token.isNullOrEmpty()) header("Authorization", "Bearer $token")
            }
            if (resp.status == HttpStatusCode.Unauthorized) {
                val newTokens = tryRefresh() ?: return emptyList()
                tokenStore.save(newTokens)
                return client.get("$API_URL/chat/sessions") {
                    header("Authorization", "Bearer ${newTokens.accessToken}")
                }.body()
            }
            return resp.body()
    ...
    override suspend fun createSession(personaId: String): ChatSession {
        try {
            val token = tokenStore.getAccessToken()
            val resp: HttpResponse = client.post("$API_URL/chat/sessions") {
                if (!token.isNullOrEmpty()) header("Authorization", "Bearer $token")
                header("Content-Type", ContentType.Application.Json.contentType)
    ...
    override suspend fun getSession(sessionId: String): ChatSession {
        try {
            val token = tokenStore.getAccessToken()
            val resp: HttpResponse = client.get("$API_URL/chat/sessions/$sessionId") {
                if (!token.isNullOrEmpty()) header("Authorization", "Bearer $token")
            }
            if (resp.status == HttpStatusCode.Unauthorized) {
                val newTokens = tryRefresh() ?: throw Exception("Unauthorized")
                tokenStore.save(newTokens)
                return client.get("$API_URL/chat/sessions/$sessionId") {
                    header("Authorization", "Bearer ${newTokens.accessToken}")
                }.body()
            }
            return resp.body()
    ...
    override suspend fun getCurrentUser(): org.julienjnnqin.luxmateapp.data.model.UserResponse {
        try {
            val token = tokenStore.getAccessToken()
            val resp: HttpResponse = client.get("$API_URL/user/me") {
                if (!token.isNullOrEmpty()) header("Authorization", "Bearer $token")
            }
            if (resp.status == HttpStatusCode.Unauthorized) {
                val newTokens = tryRefresh() ?: throw Exception("Unauthorized")
                tokenStore.save(newTokens)
                return client.get("$API_URL/user/me") {
                    header("Authorization", "Bearer ${newTokens.accessToken}")
                }.body()
            }
            return resp.body()
    ...
    override suspend fun getMessages(sessionId: String): List<ChatMessage> {
        try {
            val token = tokenStore.getAccessToken()
            val resp: HttpResponse = client.get("$API_URL/chat/sessions/$sessionId/messages") {
                if (!token.isNullOrEmpty()) header("Authorization", "Bearer $token")
            }
            if (resp.status == HttpStatusCode.Unauthorized) {
                val newTokens = tryRefresh() ?: return emptyList()
                tokenStore.save(newTokens)
                return client.get("$API_URL/chat/sessions/$sessionId/messages") {
                    header("Authorization", "Bearer ${newTokens.accessToken}")
                }.body()
            }
            return resp.body()
    ...
    override suspend fun sendMessage(sessionId: String, request: SendMessageRequest): SendMessageResponse {
        try {
            val token = tokenStore.getAccessToken()
            val resp: HttpResponse = client.post("$API_URL/chat/sessions/$sessionId/messages") {
                if (!token.isNullOrEmpty()) header("Authorization", "Bearer $token")
                header("Content-Type", ContentType.Application.Json.contentType)
                setBody(request)
            }
            if (resp.status == HttpStatusCode.Unauthorized) {
                val newTokens = tryRefresh() ?: throw Exception("Unauthorized")
                tokenStore.save(newTokens)
                return client.post("$API_URL/chat/sessions/$sessionId/messages") {
                    header("Authorization", "Bearer ${newTokens.accessToken}")
                    header("Content-Type", ContentType.Application.Json.contentType)
                    setBody(request)
                }.body()
